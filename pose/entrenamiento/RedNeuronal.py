...
def parse_args():
    parser = argparse.ArgumentParser(description='Entrenamiento de la red neuronal para las poses')
    ...
    parser.add_argument('--train-test-split', dest='train_test_split', default=0.85, type=float,
                        help='radio de entrenamiento del dataset(0-1)')
    parser.add_argument('--learning-rate', dest='learning_rate', default=0.0001, type=float,
                        help='Velocidad de aprendizaje del modelo')
    ...
    parser.add_argument('--break-entrenamiento-at', dest='break_training_at', default=0.96, type=float,
                        help='Salga del entrenamiento cuando la precisión de la prueba aumente más')
    args = parser.parse_args()
    return args


# fucion de activacion RELU
...
def evauacion(X_data, y_data, accuracy, X, y, batch_size):
    sum_at = 0.0
...
def construye_y_entrena_red(train_data, test_data, parameters, output_at):
    ...
    with tf.Session() as sess:
        ...
        for epoch_no in range(parameters['epochs']):
            ...
            train_acc = evauacion(X_train, y_train, accuracy, X, y, batch_size)
            test_acc = evauacion(X_test, y_test, accuracy, X, y, batch_size)
        ...
        os.mkdir(output_at)
        saver.save(sess, '{}/checkpointRed'.format(output_at))
        print('checkpoint guardado')

        plt.plot(range(len(train_accuracies)), train_accuracies, 'b-', label='Entrenamiento')
...
if __name__ == '__main__':
    ...
    construye_y_entrena_red(train_data, test_data, parameters, output_at)
